import { Avatar, Chip, Button, Column, Flex, Heading, Icon, IconButton, SmartImage, Tag, Text } from '@/once-ui/components';
...
export default function About() {
	const structure = [
		...
		},
		{
			...
			items: about.work.experiences.map((experience) => experience.company).filter((item): item is string => Boolean(item)),
		},
		{
			title: about.studies.title,
			display: about.studies.display,
			items: about.studies.institutions.map((institution) => institution.name).filter((item): item is string => Boolean(item)),
		},
		{
			title: about.technical.title,
			display: about.technical.display,
			items: about.technical.skills.map((skill) => skill.title).filter((item): item is string => Boolean(item)),
		},
	];
	return (
		<Column maxWidth='m'>
							...
							<Column fillWidth gap='l' marginBottom='40'>
								{about.work.experiences.map((experience, index) => (
									<Column key={`${experience.company}-${experience.role}-${index}`} fillWidth>
										<Flex vertical='center' marginBottom='4'>
											{experience.logo && <SmartImage alt={`${experience.company} logo`} src={experience.logo} width={35} height={35} radius='s' border='neutral-alpha-weak' maxWidth='40' maxHeight='40' />}
											<Flex style={{ display: 'flow' }} fillWidth paddingLeft='16' gap='8' vertical='center'>
												<Flex fillWidth horizontal='space-between' vertical='end'>
													<Text id={experience.company} variant='heading-strong-l'>
														<a key={experience.company} style={{ color: 'var(--font-weight-display-strong)' }} href={experience.link} target='_blank'>
															{experience.company}
														</a>
													</Text>
										...
										<Flex gap='8' marginBottom='m'>
											{experience.techStack && experience.techStack.filter((item): item is string => Boolean(item)).map((tech: string, index: number) => <Tag key={`${experience.company}-${index}`} variant='neutral' size='s' label={tech} />)}
										</Flex>
