...
export const HeadingLink: React.FC<HeadingLinkProps> = ({ id, level, children, style }) => {
	...
	const asTag: 'h1' | 'h2' | 'h3' | 'h4' | 'h5' | 'h6' = `h${level}`;

	return (
		<Flex style={style} onClick={() => copyURL(id)} className={styles.control} vertical='center' gap='4'>
			<Heading className={styles.text} id={id} variant={variant} as={asTag}>
				{children}
			</Heading>
			<IconButton className={styles.visibility} size='s' icon='openLink' variant='ghost' tooltip='Copy' tooltipPosition='right' />
		</Flex>
	);
};
